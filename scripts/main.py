...
class Img:
    ...
    def extract_single_cell_images(self, output_size=100, corner_distance=100):
        ...
        for cell in props_data.iterrows():
            ...
            bounding_box_mask[bounding_box_mask != label] = 0

            # Combine the image and mask into a single array
            bounding_box = np.array([bounding_box_img, bounding_box_mask, bounding_box_dapi, bounding_box_ribo, bounding_box_wga])
            if not os.path.exists(save_dir):
                os.makedirs(save_dir)
            print(f'{save_dir}/{self.name}_{label}.npy')
            np.save(file=f'{save_dir}/{self.name}_{label}.npy', arr=bounding_box)
